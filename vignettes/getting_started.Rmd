...
we recommend launching the modules using `teal` because this package does not provide launching capabilities on its own.
...
app <- teal::init(
  ...
  modules = list(tm_g_barchart_simple(label = "ADAE Analysis", x = teal::data_extract_spec(
    dataname = "ADSL",
    select = teal::select_spec(choices = teal::variable_choices(ADSL), multiple = FALSE)
  )))
)
...
`teal.modules.clinical` exports modules and needs support from other modules to provide its functionality. Actually,
...
app <- teal::init(
  ...
  modules = list(
    tm_t_summary(
      ...
      dataname = "ADSL",
      arm_var = teal::choices_selected(choices = c("ARM", "ARMCD"), selected = "ARM"),
      summarize_vars = teal::choices_selected(

...
_Disclaimer_: this vignette concerns mainly the development of minimal and stable test data and it is kept internal for feature tracking.
...
generate_adsl <- function(N = 200) { # nolint
  ...
  adsl <- tibble::tibble(
    STUDYID = rep("AB12345", N) %>% formatters_with_label("Study Identifier"),
    COUNTRY = sample_fct(
      c("CHN", "USA", "BRA", "PAK", "NGA", "RUS", "JPN", "GBR", "CAN", "CHE"),
      N,
      prob = country_site_prob
    ) %>% formatters_with_label("Country"),
    SITEID = sample_fct(1:20, N, prob = rep(country_site_prob, times = 2)),
    SUBJID = paste("id", seq_len(N), sep = "-") %>% formatters_with_label("Subject Identifier for the Study"),
    AGE = (sapply(stats::rchisq(N, df = 5, ncp = 10), max, 0) + 20) %>% formatters_with_label("Age"),
    SEX = c("F", "M") %>% sample_fct(N, prob = c(.52, .48)) %>% formatters_with_label("Sex"),
    ARMCD = c("ARM A", "ARM B", "ARM C") %>% sample_fct(N) %>% formatters_with_label("Planned Arm Code"),
    ARM = dplyr::recode(
      .data$ARMCD,
      "ARM A" = "A: Drug X", "ARM B" = "B: Placebo", "ARM C" = "C: Combination"
    ) %>% formatters_with_label("Description of Planned Arm"),
    ACTARMCD = .data$ARMCD %>% formatters_with_label("Actual Arm Code"),
    ACTARM = .data$ARM %>% formatters_with_label("Description of Actual Arm"),
    RACE = c(
      ...
      formatters_with_label("Race"),
    TRTSDTM = sys_dtm + sample(seq(0, study_duration_secs), size = N, replace = TRUE) %>%
      formatters_with_label("Datetime of First Exposure to Treatment"),
    TRTEDTM = c(TRTSDTM + study_duration_secs) %>%
      formatters_with_label("Datetime of Last Exposure to Treatment"),
    EOSDY = ceiling(as.numeric(difftime(TRTEDTM, TRTSDTM, units = "days"))) %>%
      formatters_with_label("End of Study Relative Day"),
    EOSDT = lubridate::date(TRTEDTM) %>% formatters_with_label("End of Study Date"),
    STRATA1 = c("A", "B", "C") %>% sample_fct(N) %>% formatters_with_label("Stratification Factor 1"),
    STRATA2 = c("S1", "S2") %>% sample_fct(N) %>% formatters_with_label("Stratification Factor 2"),
    BMRKR1 = stats::rchisq(N, 6) %>% formatters_with_label("Continuous Level Biomarker 1"),
    BMRKR2 = sample_fct(c("LOW", "MEDIUM", "HIGH"), N) %>% formatters_with_label("Continuous Level Biomarker 2")
  )

  # associate sites with countries and regions
  adsl <- adsl %>%
    dplyr::mutate(
      SITEID = paste0(.data$COUNTRY, "-", .data$SITEID) %>% formatters_with_label("Study Site Identifier"),
      ...
      )) %>% formatters_with_label("Geographic Region 1"),
      SAFFL = factor("Y") %>% formatters_with_label("Safety Population Flag")
    ) %>%
    dplyr::mutate(USUBJID = paste(.data$STUDYID, .data$SITEID, .data$SUBJID, sep = "-") %>%
      formatters_with_label("Unique Subject Identifier"))
  ...
  adsl <- adsl %>%
    dplyr::mutate(
      ...
      ) %>% formatters_with_label("End of Study Status")
    ) %>%
    dplyr::mutate(
      EOTSTT = .data$EOSSTT %>% formatters_with_label("End of Treatment Status")
    ) %>%
    dplyr::mutate(
      DCSREAS = ifelse(
...
generate_adae <- function(adsl = tmc_ex_adsl,
  ...
  adae <- Map(
    ...
    dplyr::mutate(
      AETERM = gsub("dcd", "trm", .data$AEDECOD) %>% formatters_with_label("Reported Term for the Adverse Event"),
      AESEV = dplyr::case_when(
        AETOXGR == 1 ~ "MILD",
        AETOXGR %in% c(2, 3) ~ "MODERATE",
        AETOXGR %in% c(4, 5) ~ "SEVERE"
      ) %>% formatters_with_label("Severity/Intensity")
    )
  ...
  adae <- adae %>%
    dplyr::mutate(
      ...
      )) %>% formatters_with_label("Outcome of Adverse Event"),
      TRTEMFL = ifelse(.data$ASTDTM >= .data$TRTSDTM, "Y", "") %>%
        formatters_with_label("Treatment Emergent Analysis Flag")
    )
  ...
  tmc_ex_adae <- adae %>%
    dplyr::mutate(
      ...
      )) %>% formatters_with_label("Action Taken With Study Treatment")
    ) %>%
    formatters_var_relabel(
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adae)[is.na(formatters_var_labels(tmc_ex_adae))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adae[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adaette <- function(adsl = tmc_ex_adsl) {
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adaette)[is.na(formatters_var_labels(tmc_ex_adaette))]),
    function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adaette[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adcm <- function(adsl = tmc_ex_adsl,
  ...
  adcm <- Map(function(id, sid) {
    ...
    Reduce(rbind, .) %>%
    `[`(c(4, 5, 1, 2, 3)) %>%
    dplyr::mutate(CMCAT = .data$CMCLAS %>% formatters_with_label("Category for Medication"))
  ...
  tmc_ex_adcm <- adcm %>%
    ...
    dplyr::mutate(
      ATC1 = paste("ATCCLAS1", substr(.data$CMDECOD, 9, 9)) %>% formatters_with_label("ATC Level 1 Text"),
      ATC2 = paste("ATCCLAS2", substr(.data$CMDECOD, 9, 9)) %>% formatters_with_label("ATC Level 2 Text"),
      ATC3 = paste("ATCCLAS3", substr(.data$CMDECOD, 9, 9)) %>% formatters_with_label("ATC Level 3 Text"),
      ATC4 = paste("ATCCLAS4", substr(.data$CMDECOD, 9, 9)) %>% formatters_with_label("ATC Level 4 Text")
    ) %>%
    dplyr::mutate(
      CMINDC = sample(c(
        "Nausea", "Hypertension", "Urticaria", "Fever",
        "Asthma", "Infection", "Diabete", "Diarrhea", "Pneumonia"
      ), dplyr::n(), replace = TRUE) %>% formatters_with_label("Indication"),
      CMDOSE = sample(1:99, dplyr::n(), replace = TRUE) %>% formatters_with_label("Dose per Administration"),
      CMTRT = substr(.data$CMDECOD, 9, 13) %>% formatters_with_label("Reported Name of Drug, Med, or Therapy"),
      CMDOSU = sample(c(
        "ug/mL", "ug/kg/day", "%", "uL", "DROP",
        "umol/L", "mg", "mg/breath", "ug"
      ), dplyr::n(), replace = TRUE) %>% formatters_with_label("Dose Units")
    ) %>%
    dplyr::mutate(
      CMROUTE = sample(c(
        "INTRAVENOUS", "ORAL", "NASAL",
        "INTRAMUSCULAR", "SUBCUTANEOUS", "INHALED", "RECTAL", "UNKNOWN"
      ), dplyr::n(), replace = TRUE) %>% formatters_with_label("Route of Administration"),
      CMDOSFRQ = sample(c(
        "Q4W", "QN", "Q4H", "UNKNOWN", "TWICE",
        "Q4H", "QD", "TID", "4 TIMES PER MONTH"
      ), dplyr::n(), replace = TRUE) %>% formatters_with_label("Dosing Frequency per Interval")
    ) %>%
    formatters_var_relabel(
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adcm)[is.na(formatters_var_labels(tmc_ex_adcm))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adcm[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adeg <- function(adsl = tmc_ex_adsl,
  ...
  lbls <- formatters_var_labels(adeg)
  adeg <- rbind(adeg, get_groups(adeg, TRUE), get_groups(adeg, FALSE)) %>%
    ...
    dplyr::ungroup()
  formatters_var_labels(adeg) <- lbls
  ...
  tmc_ex_adeg <- adeg %>%
    ...
    dplyr::ungroup() %>%
    formatters_var_relabel(BASEC = "Baseline Character Value")

  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adeg)[is.na(formatters_var_labels(tmc_ex_adeg))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adeg[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adex <- function(adsl = tmc_ex_adsl,
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adex)[is.na(formatters_var_labels(tmc_ex_adex))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adex[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adlb <- function(adsl = tmc_ex_adsl,
  ...
  adlb <- adlb %>%
    ...
    dplyr::mutate(
      ...
      )) %>% formatters_with_label("Analysis Toxicity Grade")
    ) %>%
    dplyr::group_by(.data$USUBJID, .data$PARAMCD, .data$BASETYPE) %>%
    dplyr::mutate(BTOXGR = .data$ATOXGR[.data$ABLFL == "Y"]) %>%
    dplyr::ungroup() %>%
    formatters_var_relabel(BTOXGR = "Baseline Toxicity Grade")
  ...
  adlb <- adlb %>%
    ...
    formatters_var_relabel(LBSEQ = "Lab Test or Examination Sequence Number")
  ...
  tmc_ex_adlb <- adlb %>% dplyr::mutate(
    ANL01FL = ifelse(
      ...
      "Y",
      ""
    ) %>% formatters_with_label("Analysis Flag 01 Baseline Post-Baseline"),
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adlb)[is.na(formatters_var_labels(tmc_ex_adlb))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adlb[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_admh <- function(adsl = tmc_ex_adsl,
  ...
  admh <- Map(
    ...
    Reduce(rbind, .) %>%
    `[`(c(4, 5, 1, 2, 3)) %>%
    dplyr::mutate(MHTERM = .data$MHDECOD %>% formatters_with_label("Reported Term for the Medical History"))
  ...
  admh <- dplyr::inner_join(
    ...
    dplyr::mutate(MHDISTAT = sample(
      ...
      size = dplyr::n(),
      replace = TRUE
    ) %>% formatters_with_label("Status of Disease"))
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_admh)[is.na(formatters_var_labels(tmc_ex_admh))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_admh[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adqs <- function(adsl = tmc_ex_adsl,
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adqs)[is.na(formatters_var_labels(tmc_ex_adqs))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adqs[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adrs <- function(adsl = tmc_ex_adsl) {
  ...
  adrs <- split(adsl, adsl$USUBJID) %>%
    ...
    dplyr::mutate(
      ...
      DTHFL = factor(sample(c("Y", "N"), nrow(.), replace = TRUE, prob = c(1, 0.8))) %>%
        formatters_with_label("Death Flag")
    )
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adrs)[is.na(formatters_var_labels(tmc_ex_adrs))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adrs[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_adtte <- function(adsl = tmc_ex_adsl) {
  ...
  lbls <- formatters_var_labels(adtte)
  ...
  formatters_var_labels(tmc_ex_adtte) <- c(lbls, lgTMATRSK = "Log Time At Risk")

  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_adtte)[is.na(formatters_var_labels(tmc_ex_adtte))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_adtte[names(i_lbls)]) <- common_var_labels[i_lbls]
...
generate_advs <- function(adsl = tmc_ex_adsl,
  ...
  i_lbls <- sapply(
    names(formatters_var_labels(tmc_ex_advs)[is.na(formatters_var_labels(tmc_ex_advs))]), function(x) which(names(common_var_labels) == x)
  )
  formatters_var_labels(tmc_ex_advs[names(i_lbls)]) <- common_var_labels[i_lbls]

...
To distinguish the difference, modules in  `teal.modules.clinical` have 2 separate parameters:
...
Here's an example to showcase how you can edit an output of class `listing_df`, `ElementaryTable`, or `TableTree`.
All these classes are extension of objects created using `rtables` and
can be modified with the help of `rtables` modifiers like `rtables::insert_rrow`.

```{r decorate_listing_df, message=FALSE}
library(teal.modules.clinical)
  ...
  modules = modules(
    tm_t_abnormality(
      ...
      baseline_var = variables(
        choices = any_of("BNRIND"),
        selected = "BNRIND",
        fixed = TRUE
      ),
      grade = variables(
        choices = any_of("ANRIND"),
        selected = "ANRIND",
        fixed = TRUE
      ),

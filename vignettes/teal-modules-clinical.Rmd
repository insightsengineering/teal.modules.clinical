...
`init()` is a `teal` function, `picks()`, `datasets()`, and `variables()` are `teal.picks`
...
Many `teal.modules.clinical` modules take column choices via [teal.picks::variables()].
Literal column names are usually passed as character vectors; `tidyselect` expressions
(for example [tidyselect::starts_with()]) are available when choices should be resolved
dynamically against the data. Dataset-scoped encodings use [teal.picks::picks()] with
[teal.picks::datasets()]. The `tm_t_summary()` module illustrates the `variables()` pattern:

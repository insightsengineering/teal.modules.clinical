...
testthat::test_that("template_abnormality_by_worst_grade generates correct expressions with default arguments", {
  ...
  expected <- list(
    data = quote({
      anl_labels <- formatable::var_labels(adlb, fill = FALSE)
      anl <- adlb %>%
        dplyr::mutate(
          GRADE_DIR = factor(
            case_when(
        ...
        droplevels()
      formatable::var_labels(anl) <- c(anl_labels, "Direction of Abnormality", "Highest Grade")
...
testthat::test_that("template_abnormality_by_worst_grade generates correct expressions with custom arguments", {
  ...
  expected <- list(
    data = quote({
      anl_labels <- formatable::var_labels(myadlb, fill = FALSE)
      anl <- myadlb %>%
        dplyr::mutate(
          GRADE_DIR = factor(
            case_when(

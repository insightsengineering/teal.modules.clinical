...
testthat::test_that("template_tte produces correct data expression when comparing and combining arms", {
  ...
  expected <- list(
    data = quote({
      anl <- ANL %>%
        dplyr::filter(ARM %in% c(
          "A: Drug X", "B: Placebo",
          "C: Combination"
        )) %>%
        dplyr::mutate(ARM = stats::relevel(ARM,
          ref = ""
        )) %>%
        dplyr::mutate(ARM = droplevels(ARM)) %>%
        dplyr::mutate(
    ...
    layout = quote(
      lyt <- rtables::basic_table(
        ...
        surv_time(
          vars = "AVAL",
          var_labels = paste0(
            "Time to Event (", as.character(anl$AVALU[1]),
            ")"
          ), is_event = "is_event", control = list(
            conf_level = 0.95,
            conf_type = "plain", quantiles = c(0.25, 0.75)
          ), table_names = "time_to_event"
        ) %>%
        coxph_pairwise(
          vars = "AVAL", is_event = "is_event", var_labels = c("Unstratified Analysis"),
          control = list(
        ...
        surv_timepoint(
          vars = "AVAL", var_labels = as.character(anl$AVALU[1]),
          is_event = "is_event", time_point = c(183, 365, 548),
          method = "both", control = control_surv_timepoint(
            conf_level = 0.95,
            conf_type = "plain"
          ), .indent_mods = c(
            pt_at_risk = 0L,

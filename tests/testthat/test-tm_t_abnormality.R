    ...
    layout = quote(
      lyt <- rtables::basic_table(
        ...
        rtables::split_rows_by(
          "AVISIT",
          split_label = teal::get_variable_labels(adlb, fill = FALSE)[["AVISIT"]],
          label_pos = "topleft",
          split_fun = trim_levels_to_map(map = map)
        ) %>%
        rtables::split_rows_by("PARAM",
          split_label = teal::get_variable_labels(adlb, fill = FALSE)[["PARAM"]],
          label_pos = "topleft",
          split_fun = trim_levels_to_map(map = map)
        ) %>%
        count_abnormal(
  ...
  expected <- list(
    ...
    layout = quote(
      lyt <- rtables::basic_table(
        ...
        rtables::split_rows_by(
          "PARAMCD",
          split_label = teal::get_variable_labels(adlb, fill = FALSE)[["PARAMCD"]],
          label_pos = "topleft",
          split_fun = trim_levels_to_map(map = map)
        ) %>%
        count_abnormal(

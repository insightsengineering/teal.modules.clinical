...
testthat::test_that("template_shift_by_grade generates correct expressions with default arguments", {
  ...
  expected <- list(
    data = quote({
      ...
      anl <- var_relabel(
        anl,
        PARAMCD = teal::get_variable_labels(anl, fill = FALSE)[["PARAMCD"]],
        AVISIT = teal::get_variable_labels(anl, fill = FALSE)[["AVISIT"]],
    ...
    layout = quote(
      lyt <- rtables::basic_table() %>%
        ...
        rtables::split_rows_by(
          ...
          split_label = teal::get_variable_labels(anl, fill = FALSE)[["PARAMCD"]]
        ) %>%
        rtables::split_rows_by(
          "AVISIT",
          split_fun = split_fun, label_pos = "topleft",
          split_label = teal::get_variable_labels(anl, fill = FALSE)[["AVISIT"]]
        ) %>%
        rtables::split_rows_by(
          var = "ATOXGR_GP", split_fun = split_fun,
          label_pos = "topleft",
          split_label = teal::get_variable_labels(anl, fill = FALSE)[["ATOXGR_GP"]]
        ) %>%

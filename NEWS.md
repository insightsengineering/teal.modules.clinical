...
* Updated the module to only display the checkboxes for numeric variables statistics when numeric variables are part of the selected.
...
### Bug fixes
* Fixed `summarize_logistic` implementation broken by empty string error that was reinstated in `rtables` 
...
* Updated the validation to account for the error when only one variable is selected in `tm_g_pp_patient_timeline` module.
...
* Updated `tm_t_binary_outcome` to only show selected responses in the output table when selecting "Show All Selected Response Categories".
...
* Made the grey error message in `tm_g_forest_tte` more informative when the `Endpoint` column is deselected in the left-hand encoding panel.
...
* Changed the description of the font size input for `Additional plot settings` (`tm_g_km`) to more precisely represent what it controls.
...
* Clarified labelling related to "Type of Regression" in the encoding panel from `tm_t_coxreg`.
...
* Added a check in `tm_t_coxreg` to have interactions exclusively in univariate models.
* Updated `tm_t_events_summary` to work with pooled studies.
...
* Added new module for multiple events by term, `tm_t_mult_events`.
...
* Split `teal.oncology` into two packages: `tern` and `teal.oncology` where `tern` contains all the analysis code and `teal.oncology` contains the teal modules that make the analysis
  function interactive with encodings and filtering. The `tern` package is found here: [https://github.com/insightsengineering/tern](https://github.com/insightsengineering/tern).

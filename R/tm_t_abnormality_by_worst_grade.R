...
#' @param atoxgr_var ([teal.picks::variables()]; legacy `teal.transform` objects are deprecated but still accepted)\cr
#'   object with all available choices and preselected
#'   option for the analysis toxicity grade variable.
#' @param worst_high_flag_var ([teal.picks::variables()]; legacy `teal.transform` objects are deprecated but still
#'   accepted)\cr
#'   object with all available choices and preselected option for the worst high grade flag variable.
#' @param worst_low_flag_var ([teal.picks::variables()]; legacy `teal.transform` objects are deprecated but still
#'   accepted)\cr
#'   object with all available choices and preselected option for the worst low grade flag variable.
...
tm_t_abnormality_by_worst_grade <- function(label, # nolint: object_length.
                                            ...
                                            worst_flag_indicator = teal.picks::values("Y", "Y", multiple = FALSE),
                                            add_total = TRUE,
                                            total_label = default_total_label(),
                                            drop_arm_levels = TRUE,
                                            pre_output = NULL,
  ...
  atoxgr_var <- migrate_choices_selected_to_variables(atoxgr_var)

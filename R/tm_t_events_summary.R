...
template_events_summary <- function(anl_name,
  ...
  if (length(flag_var_anl) > 0) {
    data_list <- add_expr(
      data_list,
      substitute(
        flag_var_anl_label <- teal.data::formatters_var_labels(anl[, flag_var_anl], fill = FALSE),
  ...
  if (length(flag_var_aesi) > 0) {
    data_list <- add_expr(
      data_list,
      substitute(
        flag_var_aesi_label <- teal.data::formatters_var_labels(anl[, flag_var_aesi], fill = FALSE),
...
#'       !is.na(DTHDT) ~ "Y",
#'       TRUE ~ ""
#'     ) %>% teal.data::formatters_with_label("Subject Death Flag")
#'   )
#' adae <- tmc_ex_adae
#'
...
#'   )
#'   teal.data::formatters_var_labels(dat)[names(column_labels)] <- as.character(column_labels)
#'   dat
#' }
#'

...
tm_t_exposure <- function(label,
  ...
  row_by_var <- migrate_choices_selected_to_variables(row_by_var)
  col_by_var <- migrate_choices_selected_to_variables(col_by_var)
  paramcd <- migrate_value_choices_to_picks(paramcd, multiple = FALSE)
  id_var <- migrate_choices_selected_to_variables(id_var)
  parcat <- migrate_choices_selected_to_variables(parcat)

...
template_exposure <- function(parentname,
  ...
  split_label <- substitute(
    expr = teal.data::formatters_var_labels(dataname[row_by_var], fill = TRUE),
    env = list(
      dataname = as.name(dataname),
...
#' labels <- teal.data::formatters_var_labels(adex, fill = FALSE)
#' adex <- adex %>%
...
#' teal.data::formatters_var_labels(adex) <- labels
#'
#' app <- init(
#'   data = cdisc_data(

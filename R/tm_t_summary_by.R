...
template_summary_by <- function(parentname,
  ...
  for (by_var in by_vars) {
    split_label <- substitute(
      expr = teal.data::formatters_var_labels(dataname, fill = FALSE)[[by_var]],
      env = list(
...
srv_summary_by <- function(id,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    all_q <- shiny::reactive({
      ...
      var_labels <- teal.data::formatters_var_labels(merged$anl_q()[[dataname]][, summarize_vars, drop = FALSE])

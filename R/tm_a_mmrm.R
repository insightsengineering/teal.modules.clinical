    ...
    teal.widgets::standard_layout(
      ...
      encoding = tags$div(
        ...
        bslib::accordion(
          ...
          bslib::accordion_panel(
            ...
            tags$div(
              ...
              teal.transform::module_input_ui(id = ns("aval_var"), spec = aval_var)
            ), # label:
            tags$div(
              tags$label("Select Endpoint"),
              teal.transform::module_input_ui(id = ns("paramcd"), spec = paramcd)
            ), # label:
            tags$div(
              tags$label("Visit Variable"),
              teal.transform::module_input_ui(id = ns("visit_var"), spec = visit_var)
            ), # label:
            tags$div(
              tags$label("Covariates"),
              teal.transform::module_input_ui(id = ns("cov_var"), spec = cov_var)
            ), # label:
            # todo: interaction variables
            tags$div(
            ...
            tags$div(
              ...
              teal.transform::module_input_ui(id = ns("id_var"), spec = id_var)
            ),
            selectInput(
              ns("weights_emmeans"),
        ...
        conditionalPanel(
          ...
          ui_decorate_teal_data(ns("d_lsmeans_table"), select_decorators(decorators, "lsmeans_table"))
        ),
        conditionalPanel(
          condition = sprintf("input['%s'] == '%s'", ns("output_function"), "g_mmrm_lsmeans"),
          ui_decorate_teal_data(ns("d_lsmeans_plot"), select_decorators(decorators, "lsmeans_plot"))
        ),
        conditionalPanel(
          condition = sprintf("input['%s'] == '%s'", ns("output_function"), "t_mmrm_cov"),
          ui_decorate_teal_data(ns("d_covariance_table"), select_decorators(decorators, "covariance_table"))
        ),
        conditionalPanel(
          condition = sprintf("input['%s'] == '%s'", ns("output_function"), "t_mmrm_fixed"),
          ui_decorate_teal_data(ns("d_fixed_effects_table"), select_decorators(decorators, "fixed_effects_table"))
        ),
        conditionalPanel(
          condition = sprintf("input['%s'] == '%s'", ns("output_function"), "t_mmrm_diagnostic"),
          ui_decorate_teal_data(ns("d_diagnostic_table"), select_decorators(decorators, "diagnostic_table"))
        ),
        conditionalPanel(
          condition = sprintf("input['%s'] == '%s'", ns("output_function"), "g_mmrm_diagnostic"),
          ui_decorate_teal_data(ns("d_diagnostic_plot"), select_decorators(decorators, "diagnostic_plot"))
        ),
        # End of Decorators ---
        conditionalPanel(
          condition = paste0(
...
srv_mmrm <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    anl_selectors <- selectors[setdiff(names(selectors), "cov_var")]
    ...
    anl_q <- reactive({
      ...
      data_with_tern_options_r() |>
        within(
          ...
          paramcd_var = str2lang(selectors$paramcd()$variables$selected),
          paramcd_val = selectors$paramcd()$values$selected
        ) |>
    ...
    observeEvent(map_merged(adsl_selectors)$arm_var$variables, ignoreNULL = FALSE, {
      arm_var <- map_merged(adsl_selectors)$arm_var$variables
      if (length(arm_var) == 0) {
    ...
    validate_checks <- reactive({
      ...
      input_arm_var <- map_merged(selectors)$arm_var$variables
      input_visit_var <- map_merged(selectors)$visit_var$variables
      input_aval_var <- map_merged(selectors)$aval_var$variables
      input_id_var <- map_merged(selectors)$id_var$variables
      input_paramcd <- map_merged(selectors)$arm_var$variables
      input_cov_var <- map_merged(selectors)$cov_var$variables

      all_x_vars <- c(input_arm_var, input_visit_var, input_cov_var)
    ...
    mmrm_fit <- eventReactive(input$button_start, {
      ...
      my_calls <- template_fit_mmrm(
        parentname = "ANL_ADSL",
        dataname = "ANL",
        aval_var = map_merged(anl_selectors)$aval_var$variable,
        arm_var = map_merged(adsl_selectors)$arm_var$variable,
        ref_arm = unlist(input$buckets$Ref),
        comp_arm = unlist(input$buckets$Comp),
        combine_comp_arms = input$combine_comp_arms,
        id_var = map_merged(anl_selectors)$id_var$variable,
        visit_var = map_merged(anl_selectors)$visit_var$variable,
    ...
    table_q <- reactive({
      ...
      paramcd <- unique(ANL[[map_merged(anl_selectors)$paramcd$variables]])

      basic_table_args$subtitles <- paste0(
        "Analysis Variable: ", map_merged(anl_selectors)$aval_var$variable,
        ",  Endpoint: ", map_merged(anl_selectors)$paramcd$values,

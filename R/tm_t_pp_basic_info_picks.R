...
#'
#' @export
tm_t_pp_basic_info.picks <- function(label,
                                     dataname = "ADSL",
                                     patient_col = "USUBJID",
  ...
  module(
    label = label,
    ui = ui_t_basic_info.picks,
    ui_args = args[names(args) %in% names(formals(ui_t_basic_info.picks))],
    server = srv_t_basic_info.picks,
    server_args = args[names(args) %in% names(formals(srv_t_basic_info.picks))],
...
#' @keywords internal
ui_t_basic_info.picks <- function(id,
                                  vars,
                                  pre_output,
                                  post_output) {
  ns <- NS(id)
...
#' @keywords internal
srv_t_basic_info.picks <- function(id,
                                   data,
                                   dataname,
                                   patient_col,
                                   vars,

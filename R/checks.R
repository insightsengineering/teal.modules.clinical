check_arm_ref_comp <- function(x, asl, module) {
  ...
  if (!is.null(x)) {
    ...
    Map(function(xi, var) {
      if (!is.list(xi)) {
        stop(msg, "definition for arm variable ", var, " list element needs to be lists with ref and comp elements")
      }

      rc <- names(xi)
      if (is.null(rc) || !identical(sort(rc), c("comp", "ref"))) {
        stop(msg, "definition for arm variable ", var, " nested list needs to have the elements ref and comp")
      }

      arm_levels <- unlist(xi)

      if (!all(arm_levels %in% asl[[var]])) {
        stop(msg, "definition for arm variable ", var, " refers to arm levels that do not exist in asl[[", var, "]]")
      }
    }, x, vars)
  }

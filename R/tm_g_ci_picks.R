#' teal Module: Confidence Interval Plot (teal.picks)
#'
#' This module produces a [ggplot2::ggplot()] type confidence interval plot consistent with the TLG Catalog template
#' `CIG01`. This is the `teal.picks` implementation of [tm_g_ci()].
#'
#' @inheritParams module_arguments
#' @inheritParams teal::module
#' @inheritParams template_g_ci
#' @param x_var (`variables`)\cr variable spec for the treatment variable on the x-axis.
#'   Resolved against `x_dataname`.
#' @param y_var (`variables`)\cr variable spec for the analysis value on the y-axis.
#'   Resolved against `y_dataname`.
#' @param paramcd_value (`values`)\cr value filter spec to subset rows of `y_dataname` by `PARAMCD`.
#' @param avisit_value (`values`)\cr value filter spec to subset rows of `y_dataname` by `AVISIT`.
#' @param color (`variables` or `NULL`)\cr variable spec for the color/group variable.
#'   Resolved against `x_dataname`. Pass `NULL` to omit grouping.
#' @param conf_level (`values`)\cr available confidence levels.
#' @param x_dataname (`character`)\cr name of the dataset supplying `x_var` and `color`.
#' @param y_dataname (`character`)\cr name of the dataset supplying `y_var` (filtered by PARAMCD/AVISIT).
#'
